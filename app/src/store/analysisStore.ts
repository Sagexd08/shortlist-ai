  ...
  startAnalysis: async (resumeName, resumeText, jobDescription) => {
    ...
    try {
      ...
      const mappedResult: AnalysisResult = {
        ...
        matchedSkills: data.presentSkills ? data.presentSkills.map((s: Skill) => s.name) : [],
        missingSkills: data.missingSkills ? data.missingSkills.map((s: Skill) => s.name) : [],
        extraSkills: data.extraSkills ? data.extraSkills.map((s: Skill) => s.name) : [],
    ...
    try {
      ...
      if (res.ok) {
        ...
        const mappedHistory: AnalysisResult[] = data.map((d: any) => ({
          ...
          matchedSkills: d.presentSkills?.map((s: Skill) => s.name) || [],
          missingSkills: d.missingSkills?.map((s: Skill) => s.name) || [],
          extraSkills: d.extraSkills?.map((s: Skill) => s.name) || [],
          aiSummary: d.summary || '',
